#!/usr/bin/env python3
"""
Xianyu item search module based on Playwright
"""
...
from app.utils.browser_config import get_browser_args, USER_AGENT, TIMEOUT_PAGE_LOAD_MS, TIMEOUT_NETWORK_IDLE_MS, TIMEOUT_NETWORK_IDLE_LONG_MS, TIMEOUT_BUTTON_VISIBLE_MS
...
try:
    ...
    logger.warning("Playwright not installed, will use mock data")


class XianyuSearcher:
    """Xianyu item searcher based on Playwright."""
    ...
    async def safe_get(self, data, *keys, default="N/A"):
    ...
    async def init_browser(self):
        ...
        if not PLAYWRIGHT_AVAILABLE:
            raise Exception("Playwright not installed, cannot use real search")
        ...
        try:
            if not PLAYWRIGHT_AVAILABLE:
                ...
                return {
                    'items': [],
                    'total': 0,
                    'error': 'Playwright not available, cannot get real data'
                }

            logger.info(f"Using Playwright to search Xianyu: keyword='{keyword}', page={page}, size={page_size}")
            ...
            async def on_response(response):
                """Handle API response, parse data."""
                if "h5api.m.goofish.com/h5/mtop.taobao.idlemtopsearch.pc.search" in response.url:
                    try:
                        if response.status != 200:
                            logger.warning(f"API response status error: {response.status}")
                            return

                        try:
                            result_json = await response.json()
                        except Exception as json_error:
                            logger.warning(f"Cannot parse response JSON: {str(json_error)}")
                            return
                        ...
                        logger.info(f"Got {len(items)} raw items from API")
                        ...
                        for item in items:
                            try:
                                ...
                                logger.warning(f"Failed to parse item: {str(parse_error)}")
                                continue

                    except Exception as e:
                        logger.warning(f"Response handling error: {str(e)}")

            try:
                logger.info("Visiting Xianyu homepage...")
                await self.page.goto("https://www.goofish.com", timeout=TIMEOUT_PAGE_LOAD_MS)
                ...
                await self.page.wait_for_load_state("networkidle", timeout=TIMEOUT_NETWORK_IDLE_LONG_MS)

                logger.info("Waiting for first page API response...")
                ...
                logger.info(f"Search completed, got {total_count} real items, sorted by want_count")
            ...
            error_msg = str(e)
            logger.error(f"Playwright search failed: {error_msg}")

            if "Executable doesn't exist" in error_msg or "playwright install" in error_msg:
                error_msg = "Browser not installed. Run in Docker: playwright install chromium"
    ...
    async def _get_fallback_data(self, keyword: str, page: int, page_size: int) -> Dict[str, Any]:
        """Get fallback data (mock data)."""
        logger.info(f"Using fallback data: keyword='{keyword}', page={page}, size={page_size}")
        ...
        for i in range(page_size):
            ...
            mock_items.append({
                ...
                'title': f'{keyword} related item #{item_index} [Mock]',
                ...
                'main_image': f'https://via.placeholder.com/200x200?text={keyword}+{item_index}',
                'raw_data': {
                    'mock': True,
                    'keyword': keyword,
                    'index': item_index
            ...
            for current_page in range(2, target_page + 1):
                ...
                for selector in next_button_selectors:
                    try:
                        ...
                        if await next_button.is_visible(timeout=TIMEOUT_BUTTON_VISIBLE_MS):
                            # 检查按钮是否可点击（不是禁用状态）
                        ...
                        for selector in next_button_selectors:
                            try:
                                ...
                                if await next_button.is_visible(timeout=TIMEOUT_BUTTON_VISIBLE_MS):
                                    ...
                                    if not is_disabled and not has_disabled_class:
                                        logger.info(f"Found next page button, clicking to page {page_num}...")
